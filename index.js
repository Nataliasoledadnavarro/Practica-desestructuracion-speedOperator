...
const contarLosArgumentos = (...propiedades) => {
  const longitud = propiedades.length;
  return longitud;
};
console.log(contarLosArgumentos("uno", "dos")); // 2
console.log(contarLosArgumentos("uno", "dos", "tres", "cuatro")); // 4
console.log(
  contarLosArgumentos(5, 5, 4, 1, 32, 132, 54, 3, 4, 5, 76, 45, 23, 65, 12, 17)
); // 16
...
const combinarArrays = (...propiedades) => {
  const array1 = propiedades
  console.log(array1)

  return [...array1]
  /*const resultado = propiedades.reduce((acc, curr) => {
    return acc + [...curr];
  }, []);*/
};
